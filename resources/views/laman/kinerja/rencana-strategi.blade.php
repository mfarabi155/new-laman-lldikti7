...
@section('content')
    <section class="relative pt-36 pb-20 lg:pt-48 lg:pb-28 bg-sky-100 overflow-hidden border-b border-white/50">
        ...
        <div class="container mx-auto px-6 relative z-10 text-center">
            ...
            <h1 class="text-4xl md:text-5xl font-extrabold text-primary tracking-tight mb-4 drop-shadow-sm"
                ...
                Rencana <span
                    class="text-transparent bg-clip-text bg-gradient-to-r from-secondary to-accent">Strategi</span>
            </h1>
    ...
    <section class="py-16 md:py-24 bg-[#F8FAFC] relative overflow-hidden">
        <div
            class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/clouds.png')] opacity-5 scale-125 z-0">
        </div>
            ...
            <div class="flex flex-col lg:flex-row gap-10">
                ...
                <div class="w-full lg:w-1/3">
                    <div class="sticky top-32" data-aos="fade-left" data-aos-duration="1000">
                        ...
                        <div
                            class="flex items-center space-x-3 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                        ...
                        <div class="space-y-4">
                            @forelse($pengumumanTerkini as $pengumuman)
                                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-sky-200 transition-all duration-300 group border-l-4 border-l-secondary"
                                    data-aos="fade-up">
                                    <h4
                                        class="font-bold text-primary mb-3 leading-snug group-hover:text-secondary transition-colors line-clamp-2">
                                        {{ $pengumuman->info_judul }}
                                    </h4>

                                    <p class="text-sm text-slate-500 mb-4 line-clamp-2 leading-relaxed">
                                        {{ Str::limit(strip_tags($pengumuman->info_isi), 100) }}
                                    </p>

                                    <div
                                        class="flex flex-wrap items-center text-[11px] text-slate-400 gap-y-2 mb-4 font-medium uppercase tracking-wider">
                                        <span class="mr-4 flex items-center">
                                            <i class="far fa-calendar-alt mr-1.5 text-accent"></i>
                                            {{ \Carbon\Carbon::parse($pengumuman->info_tanggal)->translatedFormat('d M Y') }}
                                        </span>
                                        <span class="flex items-center">
                                            <i class="fas fa-tag mr-1.5 text-sky-500"></i>
                                            {{ $pengumuman->bagian_nama ?? 'Umum' }}
                                        </span>
                                    </div>

                                    <a href="{{ route('pengumuman.show', $pengumuman->slug) }}"
                                        class="text-sm font-bold text-secondary group-hover:text-blue-800 flex items-center transition-colors">
                                        Baca Selengkapnya
                                        <i
                                            class="fas fa-arrow-right ml-1.5 text-[10px] transform group-hover:translate-x-1 transition-transform"></i>
                                    </a>
                                </div>
                            @empty
                                <div class="bg-white p-6 rounded-2xl border border-dashed border-slate-200 text-center">
                                    <p class="text-sm text-slate-400 italic">Belum ada pengumuman terbaru.</p>
                                </div>
                            @endforelse

                            {{-- Tombol Lihat Semua Pengumuman --}}
                            <a href="{{ route('pengumuman.index') }}"
                                class="block w-full text-center py-3 px-6 bg-slate-100 hover:bg-secondary hover:text-white text-slate-600 font-bold rounded-xl transition-all text-sm shadow-sm">

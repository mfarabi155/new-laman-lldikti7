                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
                        ...
                        <tbody class="text-slate-700 text-sm divide-y divide-slate-100">
                            @forelse($perjanjian as $index => $item)
                                <tr class="hover:bg-sky-50 transition-colors duration-200 group">
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        {{-- Memberikan warna badge berbeda untuk dokumen terbaru (index 0) --}}
                                        @if($loop->first)
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        @if($item->pk_file)
                                            <a href="{{ asset('storage/renstra/' . $item->pk_file) }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-sky-100 text-secondary hover:bg-secondary hover:text-white transition-colors shadow-sm" title="Buka / Unduh File">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic">
                                        Data perjanjian kinerja belum tersedia.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>

                ...
                <div class="lg:col-span-2">
                    ...
                    <article class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                        ...
                        <div class="p-6 md:p-8 lg:p-10 border-b border-slate-100">
                            <div
                                class="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-500 uppercase tracking-wider mb-4">
                                <span
                                ...
                                <span class="flex items-center gap-1.5"><i class="far fa-calendar-alt"></i>
                                    {{ \Carbon\Carbon::parse($pengumuman->info_tanggal)->translatedFormat('d F Y') }}</span>
                                <span class="flex items-center gap-1.5"><i class="far fa-user"></i>
                                    {{ $pengumuman->bagian_nama ?? 'Tim Kerja LLDikti' }}</span>
                            ...
                            <div class="p-6 md:p-8 lg:p-10 bg-slate-50/50 border-t border-slate-100">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    @foreach ($pengumuman->details as $file)
                                        <a href="{{ $file->info_file }}" target="_blank"
                                            class="group bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-argon-blue/30 transition-all flex items-start gap-4">
                                            <div
                                                class="w-12 h-12 rounded-lg bg-red-100 text-red-500 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                                                {{-- Bisa dibuat dinamis iconnya berdasarkan format URL, tapi default PDF paling aman --}}
                                                <i class="fas fa-file-pdf text-xl"></i>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <p
                                                    class="font-bold text-slate-700 text-sm mb-1 group-hover:text-argon-blue transition-colors line-clamp-2">
                                                    {{ $file->info_judul_file }}
                                                </p>
                                                <span
                                                    class="text-xs font-semibold text-slate-400 group-hover:text-argon-blue flex items-center gap-1">
                                ...
                                <div class="group">
                                    ...
                                    <div class="text-xs text-slate-500 flex items-center gap-3 mb-2">
                                        ...
                                        <span class="flex items-center gap-1"><i class="far fa-user"></i>
                                            {{ $terbaru->bagian_nama ?? 'Admin' }}</span>
                                    </div>
                                    ...
                                    <a href="{{ url('pengumuman/' . $terbaru->slug) }}"
                                        class="inline-flex items-center gap-1.5 text-xs font-bold text-argon-blue hover:text-argon-indigo transition-colors">

    ...
    <div class="container mx-auto px-4 lg:px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 xl:gap-8">
            ...
            @forelse($pengumuman as $item)
                {{-- CARD PENGUMUMAN --}}
                <div class="bg-white rounded-2xl shadow-sm hover:shadow-lg border border-slate-100 transition-all duration-300 flex flex-col h-full group" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    
                    <div class="p-6 md:p-8 flex flex-col flex-grow">
                        {{-- Meta Data --}}
                        <div class="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">
                            <span class="bg-argon-blue/10 text-argon-blue py-1 px-2.5 rounded-md flex items-center gap-1.5">
                                <i class="fas fa-bullhorn"></i> Info
                            </span>
                            <span class="flex items-center gap-1.5">
                                <i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($item->info_tanggal)->translatedFormat('d M Y') }}
                            </span>
                        </div>
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                            ...
                            <a href="{{ url('/pengumuman/' . $item->slug) }}" class="shrink-0 flex items-center gap-1.5 text-sm font-bold text-argon-blue group-hover:text-argon-indigo transition-colors">
            ...
            @empty
                ...
                <div class="col-span-1 md:col-span-2 lg:col-span-3 bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center" data-aos="fade-up">
                    ...
                    <p class="text-slate-500">
                        @if(request('cari'))
                            Pencarian untuk "<b>{{ request('cari') }}</b>" tidak ditemukan. Silakan coba kata kunci lain.
                            <br><a href="{{ url('/pengumuman') }}" class="inline-block mt-4 text-argon-blue font-bold hover:underline">Reset Pencarian</a>
                        @else

...
@section('content')
    ...
    <style>
        ...
        .galeri-swiper { width: 100%; padding-top: 30px; padding-bottom: 70px; }
        .galeri-swiper .swiper-slide { background-position: center; background-size: cover; width: 600px; max-width: 80%; }
        .galeri-swiper .slide-caption { opacity: 0; transform: translateY(20px); transition: all 0.5s ease; }
        .galeri-swiper .swiper-slide-active .slide-caption { opacity: 1; transform: translateY(0); }
        .galeri-swiper .swiper-button-next, .galeri-swiper .swiper-button-prev { color: white; background: rgba(255, 255, 255, 0.15); border: 1.5px solid rgba(255, 255, 255, 0.4); width: 45px; height: 45px; border-radius: 8px; backdrop-filter: blur(4px); }
        .galeri-swiper .swiper-button-next:after, .galeri-swiper .swiper-button-prev:after { font-size: 18px; font-weight: bold; }
        .galeri-swiper .swiper-pagination-bullet { background: rgba(255, 255, 255, 0.4); opacity: 1; }
        .galeri-swiper .swiper-pagination-bullet-active { background: white; }
    </style>

    <section x-data="{
        activeSlide: 1,
        loop() { setInterval(() => { this.activeSlide = this.activeSlide === 3 ? 1 : this.activeSlide + 1 }, 6000) }
    }" x-init="loop" class="relative h-[60vh] md:h-screen w-full overflow-hidden bg-sky-100">
        ...
        <div x-show="activeSlide === 3" x-transition:enter="transition ease-out duration-1000"
            ...
            x-transition:leave="transition ease-in duration-1000" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="absolute inset-0 w-full h-full" x-cloak>
            <img src="{{ asset('laman/img/carousel-3.webp') }}" class="w-full h-full object-cover md:object-fill" alt="Layanan Humanis Berintegritas 3">
        </div>

        <button @click="activeSlide = activeSlide === 1 ? 3 : activeSlide - 1" class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm transition z-20">
            <i class="fas fa-chevron-left"></i>
        </button>

        <button @click="activeSlide = activeSlide === 3 ? 1 : activeSlide + 1" class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm transition z-20">
    ...
    <div class="bg-white/70 backdrop-blur-sm text-primary py-3 overflow-hidden flex items-center marquee-container relative z-20 shadow-inner border-y border-slate-100">
        ...
        <div class="bg-accent/20 text-accent px-5 py-2 font-bold z-10 whitespace-nowrap text-xs rounded-r-full ml-4 relative border-r-2 border-white">
            INFO PENTING
        </div>
    ...
    <section class="py-24 bg-white relative">
        ...
        <div class="container mx-auto px-6 relative z-10">
            <div class="flex justify-between items-center mb-12 pb-4 border-b border-slate-100" data-aos="fade-up">
                ...
                <a href="{{ route('pengumuman.index') }}" class="text-secondary font-semibold hover:text-blue-800 transition hidden md:block group">
                ...
                @foreach ($pengumuman as $item)
                    <div data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}"
                        ...
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @php
                                $statP = \Illuminate\Support\Facades\DB::table('t_info_statistik')->where('info_id', $item->info_id)->first();
                            @endphp
                            <p class="text-[11px] text-slate-400 font-medium flex items-center bg-slate-50 px-2 py-0.5 rounded">
                                <i class="far fa-eye mr-1.5 text-secondary"></i>
                                {{ $statP ? number_format($statP->views, 0, ',', '.') : 0 }}
                            </p>
                        </div>

                        <h3 class="font-semibold text-lg text-primary leading-snug group-hover:text-secondary transition line-clamp-3 flex-grow mt-1">
                            {{ $item->info_judul }}
                        </h3>
                    </div>
                @endforeach
                    ...
                    <div data-aos="fade-right" data-aos-duration="1000"
                        ...
                        <div class="overflow-hidden relative h-96 bg-slate-100 flex items-center justify-center shrink-0">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-primary/90 to-transparent p-10 flex flex-col justify-end">
                                <span class="absolute top-6 left-6 bg-accent text-primary text-xs font-bold px-4 py-1.5 rounded-full shadow-lg">TERBARU</span>
                                ...
                                <div class="flex items-center gap-4 mb-3">
                                    ...
                                    @php
                                        $statU = \Illuminate\Support\Facades\DB::table('t_info_statistik')->where('info_id', $beritaUtama->info_id)->first();
                                    @endphp
                                    <p class="text-sm text-slate-300 flex items-center bg-black/20 px-2 py-0.5 rounded-md backdrop-blur-sm">
                                        <i class="far fa-eye mr-1.5 text-accent"></i>
                                        {{ $statU ? number_format($statU->views, 0, ',', '.') : 0 }} Views
                                    </p>
                                </div>

                                <h3 class="text-3xl font-bold text-white mb-2 leading-tight group-hover:text-accent transition duration-300 line-clamp-2">
                        ...
                        <div class="p-10 border-t border-slate-100 relative z-10 flex-grow flex flex-col justify-between">
                            ...
                            <span class="text-secondary font-semibold group-hover:text-blue-800 transition flex items-center mt-auto">
                                Baca selengkapnya &nbsp; <i class="fas fa-arrow-right text-xs group-hover:translate-x-1 transition-transform"></i>
                            </span>
                        </div>
                    </div>
                @endif
                ...
                <div class="flex flex-col space-y-5" data-aos="fade-left" data-aos-duration="1000">
                    @foreach ($beritaLainnya as $bt)
                        ...
                        <div onclick="window.location='{{ route('berita.show', $bt->slug ?: $bt->info_id) }}'"
                            ...
                            <div class="w-24 h-24 flex-shrink-0 bg-slate-50 rounded-xl overflow-hidden border border-slate-100 flex items-center justify-center">
                            ...
                            <div class="flex-1">
                                <h4 class="font-semibold text-primary group-hover:text-secondary line-clamp-2 leading-snug text-sm mb-1">
                                    {{ $bt->info_judul }}
                                </h4>
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                    ...
                                    @php
                                        $statS = \Illuminate\Support\Facades\DB::table('t_info_statistik')->where('info_id', $bt->info_id)->first();
                                    @endphp
                                    <p class="text-[10px] text-slate-400 flex items-center">
    ...
    <section class="py-28 relative bg-sky-100 text-primary overflow-hidden border-t-2 border-white rounded-t-3xl shadow-inner">
        ...
        <div class="container mx-auto px-6 relative z-10 text-center flex flex-col items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-16">
                {{-- Perguruan Tinggi --}}
                <div data-aos="flip-left" data-aos-delay="100" class="p-8 bg-white rounded-3xl border border-slate-100 shadow-soft relative border-l-4 border-accent">
                ...
                {{-- Program Studi --}}
                <div data-aos="flip-left" data-aos-delay="200" class="p-8 bg-white rounded-3xl border border-slate-100 shadow-soft relative border-l-4 border-accent">
                ...
                {{-- Dosen --}}
                <div data-aos="flip-left" data-aos-delay="300" class="p-8 bg-white rounded-3xl border border-slate-100 shadow-soft relative border-l-4 border-accent">
                ...
                {{-- Mahasiswa --}}
                <div data-aos="flip-left" data-aos-delay="400" class="p-8 bg-white rounded-3xl border border-slate-100 shadow-soft relative border-l-4 border-accent">
    ...
    <section class="py-24 relative overflow-hidden bg-[#0A2540]">
        ...
        <div class="container mx-auto px-0 md:px-6 relative z-10 text-center">
            <h2 class="text-3xl md:text-4xl font-extrabold text-white mb-6 tracking-wide drop-shadow-lg" data-aos="fade-down">
                Galeri Foto
            </h2>
                    ...
                    @foreach ($galeriRandom as $g)
                        ...
                        <div class="swiper-slide">
                            <div class="rounded-xl md:rounded-3xl overflow-hidden border-[4px] md:border-[6px] border-white shadow-2xl aspect-video bg-slate-800 flex items-center justify-center relative cursor-pointer" 
                                ...
                                @else
                                    <img src="{{ asset('laman/img/logo_lldikti.png') }}" class="block w-1/3 opacity-30 object-contain">
                                @endif
        ...
        <div class="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 relative z-10">
            ...
            <div data-aos="fade-right" data-aos-duration="1000" class="bg-white p-8 rounded-3xl border border-slate-100 shadow-soft relative group">
                ...
                <h3 class="text-2xl font-extrabold text-primary mb-8 flex items-center justify-between relative z-10">
                    ...
                    <a href="https://www.instagram.com/lldikti7/" target="_blank" class="text-xs text-secondary font-medium hover:text-blue-800">@lldikti7 &rarr;</a>
                </h3>
                <div class="w-full h-72 rounded-2xl overflow-hidden border border-slate-200 shadow-inner relative">
                    <iframe src="https://www.instagram.com/lldikti7/embed" class="w-full h-full border-0" frameborder="0" scrolling="no" allowtransparency="true"></iframe>
            ...
            <div data-aos="fade-left" data-aos-duration="1000" class="bg-white p-8 rounded-3xl border border-slate-100 shadow-soft relative group">
                ...
                <h3 class="text-2xl font-extrabold text-primary mb-8 flex items-center justify-between relative z-10">
                    ...
                    <a href="https://www.youtube.com/@lldikti7" target="_blank" class="text-xs text-secondary font-medium hover:text-blue-800">Official &rarr;</a>
                </h3>

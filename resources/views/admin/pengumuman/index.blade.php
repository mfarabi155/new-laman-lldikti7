                    ...
                    @forelse($pengumuman as $key => $item)
                        <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-slate-500 whitespace-normal max-w-[200px]">
                                {{-- Jika ada relasi atau join dengan tabel bagian, panggil di sini --}}
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- Contoh Tombol Edit di index berita --}}
                                    ...
                                    {{-- Tombol Toggle Tampil/Sembunyikan --}}
                                    <form action="{{ route('admin.berita.disable', $item->info_id) . '/disable' }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengubah status Berita ini?');">
                                        @csrf
                                        {{-- Jika status 0 (Unhide), tombolnya merah (untuk Sembunyikan) --}}
                                        {{-- Jika status 1 (Hide), tombolnya hijau (untuk Tampilkan) --}}
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-slate-400">
                                ...
                                <p class="text-sm">Belum ada data pengumuman yang tersedia.</p>
                            </td>
                        </tr>
                    @endforelse
